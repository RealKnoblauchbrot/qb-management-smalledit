...
RegisterNetEvent("qb-gangmenu:server:withdrawMoney", function(amount)
	...
	if RemoveGangMoney(gang, amount) then
		Player.Functions.AddMoney("cash", amount, 'Fraktionskonto')
		...
		TriggerClientEvent('QBCore:Notify', src, "Du hast $" ..amount.." vom Fraktionskonto abgehoben.", "success")
	else
		TriggerClientEvent('QBCore:Notify', src, "Es ist nicht genug Kapital auf dem Fraktionskonto!", "error")
	end
...
RegisterNetEvent("qb-gangmenu:server:depositMoney", function(amount)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		TriggerClientEvent('QBCore:Notify', src, "Du hast $" ..amount.." ins Fraktionskonto eingezahlt.", "success")
	else
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			if Employee.Functions.SetGang("none", '0') then
				...
				TriggerClientEvent('QBCore:Notify', src, "Mitglied rausschmeißen.", "success")
				TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , "Du wurdest aus deiner Fraktions rausgeschmissen!", "error")
			else
				TriggerClientEvent('QBCore:Notify', src, "Fehler.", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', src, "Du kannst dich nicht selber rausschmeißen", "error")
		end
	else
		...
		if player[1] ~= nil then
			...
			local gang = {}
			gang.name = "none"
			gang.label = "Keine Zugehörigkeit"
			gang.payment = 0
			gang.onduty = true
			gang.isboss = false
			gang.grade = {}
			...
			TriggerClientEvent('QBCore:Notify', src, "Mitglied gefeuert!", "success")
			TriggerEvent("qb-log:server:CreateLog", "gangmenu", "Gang Fire", "orange", Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.gang.name .. ")", false)
		else
			TriggerClientEvent('QBCore:Notify', src, "Bürger nicht im Staate.", "error")
		end
...
RegisterNetEvent('qb-gangmenu:server:HireMember', function(recruit)
	...
	if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
		TriggerClientEvent('QBCore:Notify', src, "Du hast " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " rekrutiert. Er ist nun ein " .. Player.PlayerData.gang.label .. "", "success")
		TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , "Du wurdest Rekrutiert. Willkommen zu " .. Player.PlayerData.gang.label .. "", "success")
