...
function ExploitBan(id, reason)
	...
	DropPlayer(id, 'Du wurdest Permantent gebannt. Grund: Cheating / Modding')
end
...
RegisterNetEvent("qb-bossmenu:server:withdrawMoney", function(amount)
	...
	if RemoveMoney(job, amount) then
		Player.Functions.AddMoney("cash", amount, 'Firmenkonto')
		TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Withdraw Money', "blue", Player.PlayerData.name.. "Withdrawal $" .. amount .. ' (' .. job .. ')', false)
		TriggerClientEvent('QBCore:Notify', src, "Du hast $" ..amount.. " vom Firmenkonto abgehoben.", "success")
	else
		TriggerClientEvent('QBCore:Notify', src, "Es ist nicht genug Kapital auf dem Firmenkonto!", "error")
	end
...
RegisterNetEvent("qb-bossmenu:server:depositMoney", function(amount)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		TriggerClientEvent('QBCore:Notify', src, "Du hast $" ..amount.." ins Firmenkonto abgehoben.", "success")
	else
		TriggerClientEvent('QBCore:Notify', src, "Du hast nicht genug Bargeld!", "error")
	end
...
RegisterNetEvent('qb-bossmenu:server:GradeUpdate', function(data)
	...
	if Employee then
		if Employee.Functions.SetJob(Player.PlayerData.job.name, data.grado) then
			TriggerClientEvent('QBCore:Notify', src, "Erfolgreich befördert!", "success")
			TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "Du wurdest zu" ..data.nomegrado.." befördert.", "success")
		else
			TriggerClientEvent('QBCore:Notify', src, "Rang exestiert nicht.", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Bürger nicht im Staate.", "error")
	end
	TriggerClientEvent('qb-bossmenu:client:OpenMenu', src)
end)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			if Employee.Functions.SetJob("unemployed", '0') then
				...
				TriggerClientEvent('QBCore:Notify', src, "Mitarbeiter gefeuert!", "success")
				TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , "Du wurdest gefuert. Haha..", "error")
			else
				TriggerClientEvent('QBCore:Notify', src, "Fehler..", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', src, "Du kannst dich nicht selber feuern.", "error")
		end
	else
...
RegisterNetEvent('qb-bossmenu:server:HireEmployee', function(recruit)
	...
	if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
		TriggerClientEvent('QBCore:Notify', src, "Du hast " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " eingestellt in " .. Player.PlayerData.job.label .. "", "success")
		TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , "Du wurdest eingestellt. Neuer Job: " .. Player.PlayerData.job.label .. "", "success")
